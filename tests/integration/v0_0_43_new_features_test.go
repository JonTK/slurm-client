...
func (suite *V0043NewFeaturesTestSuite) TestJobAllocation() {
	...
	ctx := context.Background()
	partitions, err := suite.client.Partitions().List(ctx, &types.PartitionListOptions{
		Limit: 5,
	})
...
func (suite *V0043NewFeaturesTestSuite) TestAssociationCreation() {
	...
	assocList, err := suite.client.Associations().List(ctx, &types.AssociationListOptions{
		Limit: 5,
	})
	if err != nil {
	...
	for i, assoc := range assocList.Associations {
		...
		suite.T().Logf("  - Association: Account=%s, User=%s, Cluster=%s", 
			assoc.AccountName, assoc.UserName, assoc.Cluster)
	}
...
func (suite *V0043NewFeaturesTestSuite) TestQoSWithTRES() {
	...
	// Test QoS listing
	qosList, err := suite.client.QoS().List(ctx, &types.QoSListOptions{
		Limit: 10,
	})
	if err != nil {
	...
	for i, qos := range qosList.QoS {
		...
		if qos.Limits != nil {
			...
			if qos.Limits.MaxCPUsPerJob != nil {
				suite.T().Logf("      MaxCPUsPerJob: %d", *qos.Limits.MaxCPUsPerJob)
			}
			if qos.Limits.MaxMemoryPerNode != nil {
				suite.T().Logf("      MaxMemoryPerNode: %d MB", *qos.Limits.MaxMemoryPerNode)
			}
			if qos.Limits.MaxNodesPerJob != nil {
				suite.T().Logf("      MaxNodesPerJob: %d", *qos.Limits.MaxNodesPerJob)
			}
		}

		// Check TRES string fields
		if qos.MaxTRESPerJob != "" {
			suite.T().Logf("    MaxTRESPerJob: %s", qos.MaxTRESPerJob)
		}
		if qos.MaxTRESPerUser != "" {
			suite.T().Logf("    MaxTRESPerUser: %s", qos.MaxTRESPerUser)
		}
	}
}
...
func (suite *V0043NewFeaturesTestSuite) TestErrorHandling() {
	...
	invalidJob := &types.JobSubmission{
		Name:      "invalid-job",
	...
	// Test invalid TRES retrieval (test context validation)
	_, err = suite.client.GetTRES(nil) // nil context
	suite.Error(err, "Should fail for nil context")
	suite.T().Logf("✓ TRES retrieval with nil context failed as expected: %v", err)

	// Test invalid partition listing
	_, err = suite.client.Partitions().List(nil, &types.PartitionListOptions{}) // nil context
	suite.Error(err, "Should fail for nil context")
	suite.T().Logf("✓ Partition listing with nil context failed as expected: %v", err)
}
...
func (suite *V0043NewFeaturesTestSuite) TestPerformanceMetrics() {
	...
	start = time.Now()
	_, err = suite.client.Partitions().List(ctx, &types.PartitionListOptions{Limit: 10})
